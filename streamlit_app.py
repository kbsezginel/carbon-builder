...
def add_perimeter(atoms):
    ana = Analysis(atoms)
    ccbonds = ana.get_bonds('C', 'C')
    st.text(f'Bonds 0 : {len(ccbonds[0])} : {ccbonds[0]}')
    patoms, bdict = detect_perimeter_by_bonds(atoms, cutoff=2)
    for a in patoms:
        if len(bdict[a]) == 1:
    ...
    ana = Analysis(atoms)
    ccbonds = ana.get_bonds('C', 'C')
    st.text(f'Bonds 1 : {len(ccbonds[0])} : {ccbonds[0]}')
    atoms = remove_duplicates(atoms, cutoff=0.5)
    ana = Analysis(atoms)
    ccbonds = ana.get_bonds('C', 'C')
    st.text(f'Bonds 2 : {len(ccbonds[0])} : {ccbonds[0]}')
    atoms = mmff_minimize(atoms)
    return atoms
...
bg_color = cols[2].color_picker("Background Color", "#ffffff")
